<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'natural' => "`natural`.`VcrId` as 'VcrId', `natural`.`VcrCodForm` as 'VcrCodForm', IF(    CHAR_LENGTH(`solicitudes1`.`VcrSol`), CONCAT_WS('',   `solicitudes1`.`VcrSol`), '') as 'VcrIdSol', DATE_FORMAT(`natural`.`VcrFecVis`, '%e/%c/%Y %l:%i%p') as 'VcrFecVis', `natural`.`VcrNomAti` as 'VcrNomAti', IF(    CHAR_LENGTH(`tipo_documento1`.`VcrTip`), CONCAT_WS('',   `tipo_documento1`.`VcrTip`), '') as 'VcrIdTip', `natural`.`VcrNoIde` as 'VcrNoIde', CONCAT_WS('-', LEFT(`natural`.`VcrCel`,3), MID(`natural`.`VcrCel`,4,3), RIGHT(`natural`.`VcrCel`,4)) as 'VcrCel', `natural`.`VcrCorEle` as 'VcrCorEle', IF(    CHAR_LENGTH(`motivo_visita1`.`VcrMotVis`), CONCAT_WS('',   `motivo_visita1`.`VcrMotVis`), '') as 'VcrIdMot', `natural`.`VcrRadSol` as 'VcrRadSol', DATE_FORMAT(`natural`.`VcrFecSol`, '%e/%c/%Y %l:%i%p') as 'VcrFecSol', `natural`.`VcrObsDat` as 'VcrObsDat', IF(    CHAR_LENGTH(`ubicacion1`.`VcrUbi`), CONCAT_WS('',   `ubicacion1`.`VcrUbi`), '') as 'VcrIdUbi', `natural`.`VcrDir` as 'VcrDir', `natural`.`VcrDirNom` as 'VcrDirNom', IF(    CHAR_LENGTH(`barrios1`.`VcrBarVer`), CONCAT_WS('',   `barrios1`.`VcrBarVer`), '') as 'VcrIdBarVe', IF(    CHAR_LENGTH(`corregimientos1`.`VcrCorr`), CONCAT_WS('',   `corregimientos1`.`VcrCorr`, ' '), '') as 'VcrIdCorr', IF(    CHAR_LENGTH(`comunas1`.`VcrCom`), CONCAT_WS('',   `comunas1`.`VcrCom`), '') as 'VcrIdCom', `natural`.`VcrLat` as 'VcrLat', `natural`.`VcrLon` as 'VcrLon', `natural`.`VcrObsUbi` as 'VcrObsUbi', IF(    CHAR_LENGTH(`fenomenos1`.`VcrFen`), CONCAT_WS('',   `fenomenos1`.`VcrFen`), '') as 'VcrIdFen', `natural`.`VcrDet` as 'VcrDet', IF(    CHAR_LENGTH(`caracteristicas_evento1`.`VcrCarFen`), CONCAT_WS('',   `caracteristicas_evento1`.`VcrCarFen`), '') as 'VcrIdCar', `natural`.`VcrOtr` as 'VcrOtr', `natural`.`VcrDesEve` as 'VcrDesEve', IF(    CHAR_LENGTH(`edificacion1`.`VcrEdi`), CONCAT_WS('',   `edificacion1`.`VcrEdi`), '') as 'VcrIdEdi', `natural`.`VcrNoPis` as 'VcrNoPis', `natural`.`VcrObrCiv` as 'VcrObrCiv', `natural`.`VcrEstEdi` as 'VcrEstEdi', IF(    CHAR_LENGTH(`afectacion1`.`VcrAfe`), CONCAT_WS('',   `afectacion1`.`VcrAfe`), '') as 'VcrIdAfe', `natural`.`VcrObsEst` as 'VcrObsEst', `natural`.`VcrDanGrav` as 'VcrDanGrav', `natural`.`VcrAnoCon` as 'VcrAnoCon', IF(    CHAR_LENGTH(`tipo_material1`.`VcrTipMat`), CONCAT_WS('',   `tipo_material1`.`VcrTipMat`), '') as 'VcrIdMat', IF(    CHAR_LENGTH(`lesiones1`.`VcrLesEst`), CONCAT_WS('',   `lesiones1`.`VcrLesEst`), '') as 'VcrIdLes', `natural`.`VcrAyuHum` as 'VcrAyuHum', `natural`.`VcrConAyu` as 'VcrConAyu', `natural`.`VcrObs` as 'VcrObs', `natural`.`VcrCapRed` as 'VcrCapRed', IF(    CHAR_LENGTH(`capacidad_reducida1`.`VcrCap`), CONCAT_WS('',   `capacidad_reducida1`.`VcrCap`), '') as 'VcrIdCap', `natural`.`VcrOtrCap` as 'VcrOtrCap', `natural`.`VcrInf0a5F` as 'VcrInf0a5F', `natural`.`VcrInf0a5M` as 'VcrInf0a5M', `natural`.`VcrInf5a12F` as 'VcrInf5a12F', `natural`.`VcrInf5a12M` as 'VcrInf5a12M', `natural`.`VcrAd12a18F` as 'VcrAd12a18F', `natural`.`VcrAd12a18M` as 'VcrAd12a18M', `natural`.`VcrAd18a28F` as 'VcrAd18a28F', `natural`.`VcrAd18a28M` as 'VcrAd18a28M', `natural`.`VcrAd28a60F` as 'VcrAd28a60F', `natural`.`VcrAd28a60M` as 'VcrAd28a60M', `natural`.`VcrAdMay60F` as 'VcrAdMay60F', `natural`.`VcrAdMay60M` as 'VcrAdMay60M', `natural`.`VcrAfr` as 'VcrAfr', `natural`.`VcrPal` as 'VcrPal', `natural`.`VcrInd` as 'VcrInd', `natural`.`VcrCau` as 'VcrCau', `natural`.`VcrGit` as 'VcrGit', `natural`.`VcrMes` as 'VcrMes', `natural`.`VcrOtrPer` as 'VcrOtrPer', `natural`.`VcrPerNsNr` as 'VcrPerNsNr', `natural`.`VcrTraCas` as 'VcrTraCas', IF(    CHAR_LENGTH(`dependencias1`.`VcrTra`), CONCAT_WS('',   `dependencias1`.`VcrTra`), '') as 'VcrIdTra1', IF(    CHAR_LENGTH(`dependencias2`.`VcrTra`), CONCAT_WS('',   `dependencias2`.`VcrTra`), '') as 'VcrdTra2', IF(    CHAR_LENGTH(`dependencias3`.`VcrTra`), CONCAT_WS('',   `dependencias3`.`VcrTra`), '') as 'VcrIdTra3', IF(    CHAR_LENGTH(`dependencias4`.`VcrTra`), CONCAT_WS('',   `dependencias4`.`VcrTra`), '') as 'VcrIdTra4', `natural`.`VcrRecLin` as 'VcrRecLin', `natural`.`VcrRecLey` as 'VcrRecLey', `natural`.`VcrRecPre` as 'VcrRecPre', `natural`.`VcrRecAut` as 'VcrRecAut', `natural`.`VcrDatPer` as 'VcrDatPer', `natural`.`VcrUsoIma` as 'VcrUsoIma', `natural`.`VcrUsoDat` as 'VcrUsoDat', `natural`.`VcrAutCor` as 'VcrAutCor', IF(    CHAR_LENGTH(`servidor_publico1`.`VcrSerPub`), CONCAT_WS('',   `servidor_publico1`.`VcrSerPub`, ' '), '') as 'VcrIdSerP', `natural`.`VcrIma` as 'VcrIma', `natural`.`VcrVideo` as 'VcrVideo', `natural`.`VcrDoc` as 'VcrDoc'",
